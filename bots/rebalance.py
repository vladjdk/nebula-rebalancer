...
def create_then_redeem(ust_used, cluster_address, imbalance_threshold):
    ...
    while x_ > imbalance_threshold:
        ...
        create_res = cluster.execute_rebalance_create(config.terra, config.wallet, cluster_address, rebalance_create_assets)
        time.sleep(7)
        ...
        inventory, weights, prices, outstanding_balance_tokens, cluster_assets = get_info_from_state(config.terra, cluster_address)
        positive_imbalances, negative_imbalances = split_imbalances(imbalances)

        spendable_capital = (float(int(tokens_received) / float(outstanding_balance_tokens) * np.dot(inventory, prices))*(1 - float(cluster_factory.query_config(config.terra)['protocol_fee_rate']))**2)/1000000 # the **2 is to
        ...
        for i in range(len(sorted_assets)):
            if sorted_assets[i].startswith("config.terra1"):
        ...
        redeem_res = cluster.execute_rebalance_redeem(config.terra, config.wallet, cluster_address,
        ...
        i_, w_, p_, _outstanding_balance_tokens, _cluster_assets = get_info_from_state(config.terra, cluster_address)
        x_ = calculate_notional_imbalance(i_, w_, p_)
